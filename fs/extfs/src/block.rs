...
use glenda::ipc::{MsgArgs, UTCB};
use glenda::protocol::device::block::{self, BlockRequest};
...
impl BlockReader {
    ...
    pub fn read_offset(&self, offset: u64, buf: &mut [u8]) -> Result<usize, Error> {
        ...
        let end_sec = (offset + buf.len() as u64 + (self.sector_size as u64) - 1) / (self.sector_size as u64);
    ...
    pub fn read_blocks(&self, sector: u64, buf: &mut [u8]) -> Result<(), Error> {
